...
- `src/full.blade.php` renders the featured article title, description, extract, image, and a small unlabelled QR code at bottom left.
...
The template reads `tfa` from the feed and prefers `titles.display`, `description`, `extract`, `thumbnail.source`, and `content_urls.desktop.page`, with sensible fallbacks for alternate feed shapes. External text and image/QR URLs are escaped before rendering. A small QR image is requested from the public QRServer endpoint only when the article has an HTTP(S) desktop or mobile page URL; the target is URL-encoded in the QR request. The QR code sits at the bottom left without a visible label or article link.

If Wikimedia returns an error, an empty response, or no featured article, the recipe shows an unavailable state. If the article has no usable thumbnail, the image panel shows a no-image fallback while title, text, and QR content remain available.
