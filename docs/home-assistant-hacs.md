...
**Completed task:** Connection-time SSRF policy — added a scheme-aware policy resolver and connector that enforce exact configured private origins, global-unicast-only unconfigured destinations, mixed-answer fail-closed behavior, per-resolution DNS-rebinding checks, literal-IP validation, TLS hostname preservation, and disabled DNS caching; focused image tests — 36 passed; full integration suite — 107 passed; dedicated-repository commit `4bd10ef`.
**Completed task:** Bounded image response transport — implemented manual redirect handling with one end-to-end timeout, then added inclusive encoded-byte limits, streamed over-limit rejection, Content-Length preflight, supported PNG/BMP magic checks, parameterized media-type validation, and unsupported/mismatched type rejection; dedicated-repository commits `99f3b2f`, `9b53846`; `python -m pytest -q tests/components/larapaper_bridge/test_image.py` — 66 passed.
