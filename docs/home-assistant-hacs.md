...
**Completed task:** Cache publication/freshness and diagnostics projections — implemented immutable last-good PNG cache, monotonic freshness boundary, UTC projections, status precedence, and safe image URL errors in dedicated-repository commits `015a11a` and `fb79dca`; focused scheduler tests passed (10), integration tests passed (66).
Remaining image retries, abandonment, and unload-race coverage remain in this item.
