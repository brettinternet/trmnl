...
**Next implementation task:** Publish the accumulated dedicated-repository state through `f57d58a`, establish the default branch, run both Home Assistant matrix lanes plus Hassfest/HACS, perform clean custom-repository installation and manual QA, publish `v1.0.0`, and record the required URLs, SHAs, and results before writing the item-level `reviewed:` marker.
