...
**Exact targets:** In `brettinternet/home-assistant-larapaper-bridge`, own `custom_components/larapaper_bridge/camera.py`, `custom_components/larapaper_bridge/diagnostics.py`, `custom_components/larapaper_bridge/manifest.json`, `custom_components/larapaper_bridge/brand/icon.png`, `custom_components/larapaper_bridge/brand/PROVENANCE.json`, `custom_components/larapaper_bridge/strings.json`, `custom_components/larapaper_bridge/translations/en.json`, root `hacs.json`, root `README.md`, `.github/workflows/test.yml`, `.github/workflows/validate.yml`, repository settings, and the `v1.0.0` release. Keep exactly one integration directory and standard layout. Root `hacs.json` contains only `name` and `homeassistant: "2026.7.0"`. Release `manifest.json` uses canonical repository/issue URLs, the settled integration metadata, version `1.0.0`, and no Pillow requirement.
...
5. v1 gate is HACS custom-repository installation plus a real v1.0.0 GitHub release; default HACS inclusion is optional post-release. Evidence: HACS publish/include guidance and OraclePackaging. Rationale: users can install a public custom repository immediately; default-list review is asynchronous and must not delay a tested release.
...
**Open questions resolved:** The sole release repository is `brettinternet/home-assistant-larapaper-bridge`. Release paths, description, topics, CI lanes/workflows, brand provenance fields, and SHA-256 check are exact above. HA-HACS-04 owns image resources and final-stop cleanup; HA-HACS-05 verifies without reimplementation. A HACS custom-repository install and `v1.0.0` release are the V1 distribution gate; default-list inclusion is optional later.

**Acceptance:**

* A fresh public clone has exactly one integration, root minimal `hacs.json`, canonical manifest URLs, version `1.0.0`, no Pillow requirement, `strings.json` and `translations/en.json`, the exact brand/provenance files and fields, exact description/topics, and CI-confirmed PNG dimensions plus recomputed SHA-256 equality.
...
* Repository settings use the exact description and four topics; both exact workflow files pass; README, issue link, release notes, tag `v1.0.0`, and published release are present; the custom-repository install succeeds from that release. Verify HA-HACS-04's domain session/executor/admission reuse, final-stop close/shutdown, and busy-to-`image_conversion_failed` behavior without reimplementing it. No default HACS inclusion is asserted.

**Pending verification:** Before implementation is called complete, record the public repository URL and commit SHA, CI run URLs for both HA lanes plus Hassfest/HACS Action, clean-install version and HACS custom-repository result, manual QA date/HA version, and GitHub release URL. Verify that the release artifact contains no unrelated recipes and that the brand provenance record matches the checked-in bytes.
**Review finding (2026-07-11):** Reviewed the accumulated dedicated-repository state through `1c2cb62`, `943cb71`, `b17c73c`, `d5aeb3d`, `cfeb149`, `22caaf5`, `fa5f7c2`, and review-fix `f57d58a` across camera, diagnostics, setup wiring, packaging, CI, and shared image resources. Found that one image operation remembered only its latest abandoned generation; a delayed older generation could submit conversion after a later cycle. `f57d58a` fences all abandoned generations and adds a regression test. Verification: `python -m pytest -q` — 182 passed; focused release/image tests — 112 passed. No valid item-level `reviewed:` marker is written yet because the release acceptance remains open: the public repository has no refs/default branch, CI has no runs, and no `v1.0.0` tag/release, clean-install, or manual-QA evidence exists.
**Next implementation task:** After owner approval of the repository license, run clean custom-repository installation and manual QA, publish tag/release `v1.0.0`, and record the required URLs, SHAs, and results before writing the item-level `reviewed:` marker.
blocked: HA-HACS-05 — repository code-license choice requires owner approval; tried: added provisional MIT `LICENSE` and passed HACS validation; unblock: owner explicitly selects MIT or another HACS-acceptable repository license.
